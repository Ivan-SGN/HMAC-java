...
HTTP-сервис для подписи и проверки целостности сообщенийй
...
Секрет должен быть сгенерирован и закодирован в формате Base64.
...
3. Сгенерировать `secret` (см. блок «Генерация секрета» ниже) и подставить его в `config.json`.
